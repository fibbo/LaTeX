...
\begin{document}
\begin{titlepage}
\hspace*{\fill}\includegraphics[width=0.4\textwidth]{uzh_logo_d_pos}\vspace{2cm}\hspace*{\fill}
...
Dieses Dokument dient als kleines Nachschlagewerk für die Arbeit mit \LaTeX. Es ist bei weitem nicht komplett und kratzt höchstens an der Oberfläche der angesprochenen Themen. Um zum Beispiel mehr über die einzelnen Pakete zu erfahren lohnt es sich immer deren Dokumentation durchzulesen um die Funktionalitäten des jeweiligen Pakets besser zu verstehen. Alternativ kann man online das \LaTeX\ Wikibook (\url{https://en.wikibooks.org/wiki/LaTeX}) als erweitertes Kompendium konsultieren. Aber auch das deckt nur einen Bruchteil von \LaTeX ab.

Bei spezifischen Fragen zu \LaTeX empfiehlt es sich auf \url{tex.stackexchange.com} nach möglichen Lösungen zu suchen, oder falls nicht vorhanden auch selber eine Frage zu stellen.
...
\begin{itemize}
\item \verb+\documentclass[10pt]{article}+ definiert die Dokumentvorlage (article, report, book). 10pt gibt die Standardschriftgr"osse des Dokumentes an.
\item \verb+\usepackage[•]{•}+ l"adt sogenannte Pakete die gewisse Optionen hinzuf"ugen oder Dinge vereinfachen. In diesem Fall erlauben Zeile 2 und 3 die direkte Eingabe von Umlauten (anstatt \verb+"a+)
\item \verb+\begin{document} \end{document}+ stellen den tats"achlichen Anfang und das Ende des Dokumentes dar. Alles was vor oder nach diesen Befehl auftaucht, wird nicht in dem pdf erscheinen.
\item \% erlaubt das Setzen von Kommentaren. Alles nach einem \%-Zeichen wird ignoriert und sind nur im Editor, aber nicht im Ausgabepdf ersichtlich.
\end{itemize}

Der Code der \textbf{VOR} \verb+\begin{document}+ kommt, nennt sich \textit{Pr"aambel}. Dorthin geh"oren fast alle Befehle, die nicht direkt zum Inhalt geh"oren (also \verb+\usepackage[•]{•}+-Befehle zum Beispiel oder setzen von Dokumentoptionen (Kopf-/Fusszeilenlayout).
...
\begin{itemize}
\item \verb+\usepackage[ngerman]{babel}+ ist das Sprachpaket f"ur \LaTeX. Ist zum Beispiel daf"ur zust"andig, dass W"orter richtig getrennt werden.
\item \verb+\title{•}+ definiert den Titel. Dieser Befehl alleine generiert aber noch kein Titelblatt - erst das setzen von \verb+\maketitle+ f"ugt tats"achlich eine Titelseite ein.
\item \verb+\author{•}+ definiert den oder die Autoren. Mehrere Authoren werden mit \verb+\and+ verkn"upft.
\item \verb+\\+ erzeugt eine neue Zeile.
\item \verb+\date{\today}+ f"ugt das heutige Datum auf der Titelseite hinzu. \verb+\today+ kann mit einem beliebigen String ersetzt werden, der dann auf der Titelsite eingef"ugt wird.
\item \verb+\maketitle+ Titelseite wird dort im Dokument erstellt, wo dieser Befehl steht. Wenn man also die Titelseite als erstes Blatt will, setzt man diesen Befehl direkt nach \verb+\begin{document}+
\item \verb+\newpage+ f"ugt eine neue Seite ein.
\end{itemize}
